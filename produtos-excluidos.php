...
<html lang="en" dir="ltr">
  <head>
    ...
    <script>
      function setExcluir(id) {
          $.ajax({
            url: "php/produto-deleta.php",
            type: "GET",
            data: "id="+id,
            dataType: "html"
      ...
      function setRestaura(id) {
          $.ajax({
            url: "php/produto-restaura.php",
            type: "GET",
            data: "id="+id,
            dataType: "html"
          }).done(function(resposta) {
                    ...
                    <table class="table card-table table-vcenter text-nowrap">
                      ...
                      <tbody>
                        <?php 
                          ...
                          $selec = 'SELECT pro.id, pro.nome, CONCAT(\'R$ \',REPLACE(pro.valor,\'.\',\',\')) AS valor, pro.estoque,CASE WHEN DATE_FORMAT(ven.data,\'%d/%c/%Y\') IS NULL THEN \'Nunca Vendido\' ELSE DATE_FORMAT(ven.data,\'%d/%c/%Y\') END AS \'data\',CASE WHEN ven.quantidade*ven.valor IS NULL THEN \'Nunca Vendido\' ELSE CONCAT(\'R$ \',REPLACE(ven.quantidade*ven.valor,\'.\',\',\')) END AS \'total\' FROM produtos AS pro LEFT JOIN vendas AS ven ON pro.id = ven.id_produto WHERE pro.lixeira = \'true\' ORDER BY pro.id';
                          $produtos = $db->query($selec)->fetchAll();

                          foreach ($produtos as $produto) {
                            echo '<tr>';
                              echo '<td name="id" id="id"><span class="text-muted">' . $produto['id'] . '</span></td>';
                              echo '<td>' . $produto['nome'] . '</td>';
                              echo '<td>' . $produto['valor'] . '</td>';
                              echo '<td>' . $produto['estoque'] . '</td>';
                              echo '<td><a class="icon" title="Restaurar" href="javascript:setRestaura('.$produto['id'].')"><i class="fe fe-arrow-up"></i></a></td>';
                              echo '<td><a class="icon" title="Excluir" href="javascript:setExcluir('.$produto['id'].')"><i class="fe fe-trash"></i></a></td>';
                            echo '</tr>';
                          }
                        ?>
                      </tbody>
